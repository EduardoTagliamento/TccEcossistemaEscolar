...
export default class VerificacaoEmail {
  ...
  set VerificacaoCodigo(value: string) {
    ...
    // Validar tamanho: 6-10 caracteres
    if (codigo.length < 6 || codigo.length > 10) {
      throw new Error("VerificacaoCodigo deve ter entre 6 e 10 caracteres.");
    }
...
- ✅ Código 6-10 caracteres alfanuméricos
...
### **TAREFA 4: Adicionar método ao SendBrevoEmailService**

**Arquivo:** `backend/external/SendBrevoEmailService.ts` (ATUALIZAR)
...
export default class VerificacaoEmailMiddleware {
  ...
  validateCodigoBody = (request: Request, _response: Response, next: NextFunction) => {
    ...
    if (codigo.length < 6 || codigo.length > 10) {
      throw new ErrorResponse(400, "Erro na validação de dados", {
        message: "O código deve ter entre 6 e 10 caracteres.",
      });
    }
...
### **Já Instaladas:**
- ✅ `axios` - HTTP client (usado pelo SendBrevoEmailService)
...
| 2 | Repository | 2-3h |
| 3 | Service | 3-4h |
| 4 | Atualizar SendBrevoEmailService | 30min |
...
- [ ] Entity: `verificacao-email.model.ts` criada
- [ ] Repository: `verificacao-email.repository.ts` criado (6 métodos)
- [ ] Service: `verificacao-email.service.ts` criado (3 métodos públicos)
- [ ] Método `sendVerificationEmail()` adicionado ao SendBrevoEmailService
- [ ] Método `verificarEmail()` adicionado ao UsuarioDAO
- [ ] Middleware: `verificacao-email.middleware.ts` criado
- [ ] Controller: `verificacao-email.controller.ts` criado (3 endpoints)
- [ ] Routes: `verificacao-email.routes.ts` criado (factory pattern)
- [ ] Server.ts atualizado (import + registro de rota)
- [ ] Documentação: `verificacao-email-api.md` criada
- [ ] Variáveis de ambiente configuradas (.env)
- [ ] Tabela `verificacao_email` criada no MySQL
...
- **SendBrevoEmailService:** `backend/external/SendBrevoEmailService.ts`
- **Código de exemplo:** `backend/external/testSendBrevoEmail.ts`
