...
- ~~Modal "+" sem backdrop pré-preenchido: formulários ainda não leem os query params~~ **corrigido (verificado 2026-07-27):** `ConteudoForm`/`TarefaForm`/`ProvaAgendadaForm` já leem `MateriaGUID`/`TurmaGUID`/`CategoriaGUID` (via prop quando embutido no `NovoItemModal`, ou query string na tela `/cadastro` standalone) e se auto-preenchem. Ponta a ponta funcional.
- **Progresso de vídeo via YouTube (link) — resolvido (2026-07-28):** o player deixou de ser um `<iframe>` cru e passou a ser instanciado via YouTube IFrame Player API (`frontend/lib/youtube/youtubeIframeApi.ts` carrega `https://www.youtube.com/iframe_api` sob demanda, singleton). `VisualizadorItemModal` escuta `onStateChange`: enquanto `PLAYING`, reporta progresso a cada 5s via `getCurrentTime()/getDuration()` (mesmo endpoint `registrarProgressoVideo` já usado pelo `<video>` nativo); em `ENDED`, força 100%. Sem dependência nova — só o script oficial do Google, sem SDK npm.
- ~~Badge de pendência na navbar: não implementado~~ **corrigido (verificado 2026-07-27):** `DashboardNavbar.tsx` já chama `verificarPendenciaAgregada` e mostra o indicador vermelho no ícone "Matérias".
- **Gerenciar categorias no "+" da tela de seleção de turma (`GerenciarCategoriasModal`):** implementado — ícone de pasta ao lado do lápis/"+" em `.../materias/[materiaGUID]/turmas`, abre modal com criação de categoria "geral" (bulk insert via `criarCategoriaGeral`, uma linha de `categoriaconteudo` por turma ativa do professor naquela matéria) e drag-and-drop nativo (mesmo padrão HTML5 da tela de categorias) pra mover itens sem categoria entre categorias e reordenar categorias (`reordenarCategoriasGerais`). Categorias aqui são sempre as "do professor" (generalizadas pra todas as turmas); a função futura de categoria por-representante-de-turma não passa por essa tela.
  - **Condensado (2026-07-27):** `buscarBoardGeral` agrupa Conteúdo/Prova (fan-out real, mesmo `ConteudoGUID`/`ProvaAgendadaGUID` em N `ConteudoTurma`/`ProvaAgendadaTurma`) numa única linha por item, mostrando a lista de turmas onde existe; item que só está em 1 turma não aparece nesta tela (é organização daquela turma específica, feita na tela de categorias da própria turma). Tarefa é sempre de 1 turma só por natureza (decisão #4) — não tem duplicata pra condensar, continua listada normalmente. Mover um item condensado (`moverItemBoardGeral`) agora aplica a categoria em TODAS as turmas onde aquele Conteúdo/Prova existe de uma vez (resolvido no backend a partir da alocação ativa do professor, não mais confiando num único `TurmaGUID` vindo do cliente).
- **Categoria por nome no `ConteudoForm`/`ProvaAgendadaForm` (2026-07-27):** os dois formulários de `/cadastro` exigiam "exatamente 1 turma marcada" pra escolher/criar categoria, porque o campo salvo era `CategoriaGUID` (uma linha por turma). Trocado por um campo de **nome** (`<input list>` com datalist das categorias "gerais" do professor naquela matéria, vindo do mesmo `buscarBoardGeral` do "Gerenciar categorias") — funciona com qualquer quantidade de turmas selecionadas. No submit, o nome é resolvido/criado por turma via `resolverCategoriaPorNomeParaTurmas` (novo método em `categoriaconteudo.service.ts`, endpoint `POST /api/categoria-conteudo/geral/resolver`), que reaproveita o mesmo helper de resolver-ou-criar do board geral (`#resolverOuCriarCategoriaGeral`), mas pra uma lista arbitrária de turmas (não necessariamente todas as turmas ativas do professor). Isso só cobre a categoria "do professor" (tier generalizado) — nunca cria/edita categoria de representante.
  - **Modelo de dois tiers confirmado com o usuário:** categoria "do professor" (mesmo nome replicado em N turmas, aparece no board geral porque este é sempre consultado com `UsuarioCPF` = o próprio professor) vs. categoria "do representante" (turma-específica, só aparece na tela de categorias daquela turma — nunca no board geral, já que o `UsuarioCPF` da linha seria do representante, não do professor). Nenhuma coluna nova foi necessária — a separação já cai inteiramente de **quem é o dono da linha (`UsuarioCPF`)** + **qual tela filtra por CPF (board geral) e qual não filtra (tela de categorias da turma)**.
  - **Gap de permissão fechado:** `criarCategoria` (`POST /api/categoria-conteudo`, categoria de 1 turma só) não validava alocação nenhuma antes — qualquer usuário autenticado conseguia criar categoria em qualquer matéria/turma. Agora exige professor ativamente alocado naquela matéria/turma (mesma checagem usada em `reordenarItens`/`moverItemBoardGeral`). Representante ainda não está liberado aqui (função futura).
  - **Bug real encontrado e corrigido no `TarefaForm` (2026-07-27):** criar tarefa com categoria dava 403 "Categoria inválida" mesmo com exatamente 1 turma marcada. Causa: o seletor de matéria do `TarefaForm` deduplica por matéria (`materiasUnicas`), guardando só UM `MatProfTurGUID` "representante" — que corresponde a uma turma arbitrária (a primeira encontrada pro professor lecionando aquela matéria), não à turma de quem foi efetivamente marcado no modal de alunos. Como `TarefaAcademica.matXprofXturxescGUID` é o campo que ancora a tarefa numa turma (é sempre de 1 turma só — decisão #4), a validação de categoria comparava contra a turma ERRADA sempre que o "representante" não coincidia com a turma de fato usada. Esse bug preexiste à sessão atual — só nunca tinha aparecido porque nada validava a turma antes da categoria por turma existir.
    - **Correção:** `GET /api/professor/turmas-alunos` (`ProfessorService.buscarTurmasAlunos`) agora devolve o `MatProfTurGUID` PRÓPRIO de cada turma (antes só devolvia `TurmaGUID`/`TurmaNome`/`alunos`). O `TarefaForm` agrupa os alunos marcados por turma (`obterGruposPorTurma`) e faz uma chamada `POST /api/tarefa/batch` **por turma**, cada uma com o `matXprofXturxescGUID` certo daquela turma — em vez de uma única chamada com o GUID "representante" errado. Como efeito colateral (correto): tarefa compartilhada com alunos de N turmas agora forma grupos por turma, não misturando alunos de turmas diferentes no mesmo grupo — mais consistente com "tarefa é sempre de 1 turma só".
    - Categoria no `TarefaForm` generalizada por nome, mesma mudança aplicada ao `ConteudoForm`/`ProvaAgendadaForm` acima — não exige mais exatamente 1 turma marcada.
- ~~Card individual de matéria/turma sem indicador de pendência: telas não chamam `tem-pendencia`~~ **corrigido (verificado 2026-07-27):** `/materias` e `/turmas` já chamam `MateriasModuloAPI.temPendencia` por card e populam a prop `temPendencia` do `MateriaTurmaCard`.
- ~~Formulário de tarefa/prova dentro do visualizador é básico: anexo de entrega só tem aviso, sem upload embutido~~ **corrigido (2026-07-27), completado (2026-07-28):** `VisualizadorItemModal` tem upload embutido pra tarefa digital (`<input type="file">` → `uploadAnexo` genérico + `POST /tarefa/:GUID/anexo-entrega`). Enviar o anexo marca `TarefaFeito=true` automaticamente no backend (`enviarAnexoEntrega`) — não precisa mais de um clique manual separado em "marcar como feito" depois de já ter enviado o arquivo. A avaliação do professor mostra o **nome do aluno** (`MatriculaAtribuidaDTO.AlunoNome`, resolvido via JOIN `matricula`+`usuario` em `tarefaacademica.service.ts`, sem N+1) em vez do GUID cru da matrícula, com status visual (entregue/atrasada/pendente) e data de entrega. Também corrigido de quebra: `GET /api/tarefa/:guid` sempre devolvia a lista de atribuições de **todos** os alunos da turma (uma linha de `TarefaAcademica` é compartilhada por N alunos); agora aceita `?minhaMatricula=true`, usado pelo aluno pra ver só a própria atribuição — sem isso, qualquer aluno veria notas/entregas dos colegas pela mesma tarefa. Segue sem cobrir 100% a tela `/tarefas/[tarefaGUID]` (ex.: pendências individuais de grupo, transferência de liderança) — o visualizador é focado no essencial de uma tarefa/prova dentro do fluxo de categoria.
  - **Professor só corrige após o envio do aluno:** `avaliarTarefa` agora rejeita (400) se `TarefaFeito` ainda for `false` — antes disso não havia checagem nenhuma, o professor podia atribuir nota a uma entrega inexistente. No `VisualizadorItemModal`, o campo de nota por aluno só aparece pra quem já entregou; quem não entregou mostra "Aguardando entrega" (ou "Prazo vencido — nota 0 automática", se já passou do prazo e o scheduler zerou).
  - **Estado "atrasado" (cadeado vermelho) corrigido pra não sumir depois do auto-zero:** o scheduler de nota automática (`tarefaacademicanota.scheduler.ts`) zera `TarefaNota=0` quando o prazo vence sem entrega, mas isso fazia `buscarCategoriasCompletas` classificar o item como "avaliado" (verde/vermelho proporcional, SEM cadeado) assim que a nota deixava de ser `NULL` — o cadeado só sobrevivia até o próximo tick do scheduler (até 5 min). Corrigido: o estado agora distingue nota automática (`TarefaAvaliadoPorCPF IS NULL`) de nota manual de verdade (`TarefaAvaliadoPorCPF` = CPF do professor) — só a manual vira "avaliado"; a automática continua "atrasado" (100% vermelho + cadeado) permanentemente, já que o aluno nunca entregou.
- **Exportar estatísticas para planilha (.xlsx) — concluído (2026-07-28), apesar do commit "pela metade":** o painel de estatísticas do `VisualizadorItemModal` (aberto pelo ícone de gráfico, só professor) ganhou um botão "Exportar" ao lado da média da turma, visível quando há pelo menos 1 aluno no ranking. `exportarEstatisticasExcel` monta as linhas (Posição/Nome/MatriculaGUID/Percentual/Nota) a partir do mesmo `EstatisticasItem` já buscado pra tela e chama `exportarParaExcel` (`frontend/lib/exportarExcel.ts`, novo helper client-side com `XLSX.utils.json_to_sheet` + `XLSX.writeFile`, reaproveitando a lib `xlsx` que já era dependência do projeto pra leitura de upload em `BaseUploadPlanilha.tsx` — essa é a primeira vez que é usada pra escrita). Nome do arquivo é o título do item (tarefa/conteúdo/prova) sanitizado (`estatisticas-<titulo-sanitizado>.xlsx`). Ícone `download` novo em `Icon.tsx`. Não depende de endpoint novo — usa o `GET /api/categoria-conteudo/estatisticas/:tipo/:itemGUID/:turmaGUID` (seção 5.7) que já existia.
...
- **Backfill de categoria (seção 4.3):** script escrito em `backend/database/migrations/2026-07-27-backfill-categoria-turma.ts` (2026-07-27) — duplica cada categoria antiga (`TurmaGUID NULL`) por turma ativa do professor, reatribui `conteudoturma.CategoriaGUID` a partir do `conteudo.CategoriaGUID` legado, e remove a linha antiga já coberta. Tem modo `--check` (só leitura/relatório) e `--apply` (grava de verdade); reporta categorias "órfãs" (professor sem alocação ativa na matéria) pra revisão manual antes de travar `TurmaGUID NOT NULL`. **Ainda não foi executado contra produção** — `backend/database/mysql.ts` cai pro banco de produção (Railway) por padrão se as env vars `DB_*` não estiverem setadas, então quem rodar precisa confirmar o ambiente antes. Rodar primeiro com `--check`, revisar o relatório, só depois `--apply`.
