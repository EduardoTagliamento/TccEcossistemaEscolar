...
- [x] #24 Endpoints: submissão de projeto — `vincularAnexoSubmissao` + `submeterProjeto` em `grupoprojeto.service.ts` (com guarda `Serviço mal configurado` para `anexoDAO`/`relacaoAnexosDAO` opcionais), controller (`vincularAnexoSubmissao`, `submeterProjeto`), rotas `POST /:grupoGUID/submissao/anexo` e `POST /:grupoGUID/submeter` (`routes/grupoprojeto.routes.ts`), `VincularAnexoBodySchema` + `validateVincularAnexoBody` no middleware. Corrigido também um gap real encontrado durante a implementação: `HistoricoTipo` no entity TS não incluía `'Submissao'` (a migration do banco já tinha, mas o TS não — causaria erro de compilação e falha em runtime). Migration `2026-08-21-notificacao-projeto-submetido.ts` semeia `notificacaotipo`/`notificacaotipofuncao` (`projeto_submetido`, público Professor+Direção) — **rodada em produção com sucesso pelo usuário em 2026-08-21**. `npx tsc --noEmit -p .` (backend) limpo.
- [x] #25 Frontend: `GerenciarGrupoModal` — seção "Personalizar grupo" (nome/cor/upload, gated por `MinhasPermissoes.PodePersonalizarGrupo`) + checkboxes de permissão por membro (`PodeExcluirMensagens`, `PodePersonalizarGrupo`), visíveis só pra quem concede (Representante/Líder). Exigiu expor 2 coisas que o backend ainda não retornava: `ConversaDetalheDTO.MinhasPermissoes`/`ConversaGrupoCorFundo`/`ConversaGrupoImagemUrl` (já existiam — só não estavam no client TS) e `MembroDTO.Permissoes` por membro (novo — sem isso o líder não tinha como ver o estado atual de cada checkbox; exigiu `MembroPermissoes` na query de `findMembrosComNome`).
- [x] #26 Frontend: páginas de grupoprojeto/grupotarefa — 3 checkboxes (expulsar/atualizar/submeter) em grupoprojeto e 2 (expulsar/atualizar) em grupotarefa, visíveis só pro líder; ações "Expulsar"/"editar nome" agora usam a permissão delegada (`MinhasPermissoes`) em vez de checar só `souLider`; "Transferir liderança" continua líder-only (fora de escopo de delegação, como decidido). Painel "Submissão do projeto" em grupoprojeto: upload via `/api/anexo` → `vincularAnexoSubmissao` → `submeterProjeto`, com badge "já submetido" quando `GrupoProjetoSubmetidoEm` estiver setado.
- [x] #27 Frontend: API clients novos — `conversa.api.ts` (`atualizarPersonalizacaoGrupo`, `atualizarPermissaoMembroChat`), `grupoprojeto.api.ts` (`atualizarPermissaoMembro`, `vincularAnexoSubmissao`, `submeterProjeto`), `grupotarefa.api.ts` (`atualizarPermissaoMembro`) + hooks React Query correspondentes.
- [x] #28 Verificação final (parcial) — `npx tsc --noEmit -p .` (backend) e `cd frontend && npx tsc --noEmit -p .` limpos após #25-27. Descoberto e corrigido no caminho: `GrupoProjetoComMembrosDTO`/`GrupoTarefaComMembrosDTO` não expunham `MinhasPermissoes` nem `Permissoes` por membro (sem isso o frontend não tinha como saber o que mostrar sem tentar-e-receber-403) — adicionado `#comMinhasPermissoes` em `grupoprojeto.service.ts`/`grupotarefa.service.ts` e `Permissoes` por linha de membro nos repositórios; `buscarGrupo`/`listarGruposDoProjeto` (grupoprojeto) passaram a exigir `usuarioGUID`. **Pendente**: teste manual ponta a ponta em produção (seção 6 do plano — trocar cor/foto do grupo, delegar/revogar cada permissão, fluxo completo de submissão) e rodar a migration `2026-08-21-notificacao-projeto-submetido.ts` (bloqueada pelo classifier de auto-mode, precisa ser rodada manualmente pelo usuário).
- [x] #29 Grupo de Projeto ganha chat associado — implementado (backend + frontend), `tsc --noEmit` limpo nos dois. Decisão do usuário (2026-08-21): grupos já existentes ganham chat retroativamente (backfill), não só os criados a partir de agora.
  - Migration `2026-08-21-chat-grupoprojeto.ts`: `ALTER ... MODIFY ConversaGrupoTipo ENUM('Turma','Tarefa','Projeto')` + backfill (cria `conversa`/`conversa_grupo`/`conversa_grupo_membro` pra todo `grupoprojeto` sem chat, líder como `Lider` + membros existentes). **Rodada em produção com sucesso pelo usuário em 2026-08-21** — enum alterado, 1 grupo de projeto existente ganhou chat retroativamente via backfill.
  - `conversa-grupo.model.ts`, `conversa-grupo.repository.ts`, `conversa.service.ts`, `conversa-permissao.service.ts` (`#resolverEscolaGUID` + `#notificarPromocao`, esta última refatorada pra reusar `#resolverEscolaGUID` em vez de duplicar a query pela 3ª vez), `mensagem.service.ts` (`#notificarMensagemGrupo`), `permissao-granular.helper.ts` (`resolverPermissaoChat`) — todos com o 3º branch pra `'Projeto'` (espelha Tarefa: líder único, sem Representante/Vice).
  - `conversa-grupo.service.ts`: `criarConversaParaGrupoProjeto`/`adicionarMembroGrupoProjeto`/`removerMembroGrupoProjeto`/`encerrarConversaGrupoProjeto`/`transferirLiderGrupoProjeto`, espelhando os métodos de Tarefa 1:1.
  - `grupoprojeto.service.ts`: novo param opcional `conversaGrupoService?`, wired em `criarGrupo`/`entrarGrupo`+`adicionarMembro` (via `#entrarNoGrupoComLimiteDeVagas`)/`sairGrupo`/`expulsarMembro` (3 sub-casos: dissolve, promove+expulsa líder, expulsa membro comum)/`transferirLideranca`. `routes/grupoprojeto.routes.ts` instancia e injeta.
  - Achado real durante a implementação: `ConversaDAO.findAllByUsuarioGUID` tinha um JOIN condicional (`filtroGrupo`/`COALESCE`) que só sabia resolver `EscolaGUID` pra Turma/Tarefa — sem o 3º branch pra Projeto, toda conversa de grupo de Projeto ficaria invisível na lista do `/chat` sempre que filtrada por escola (que é o caso normal de uso). Corrigido.
  - Decisão de design (não estava no plano original, resolvida durante a implementação): em vez de construir UI de chat inline na página de grupo de projeto, reaproveitei a página `/chat` já existente — `GrupoProjetoComMembrosDTO` ganhou `ConversaGUID`, e a página de grupo mostra um link "Ir para o chat do grupo" pra `/dashboard/{escola}/chat?conversa={guid}` (mesmo padrão que notificações já usam). Isso é o "generalizar por ConversaGUID em vez de duplicar" que o plano original já sugeria como caminho preferido.
  - `GerenciarGrupoModal.tsx`: corrigido um bug que essa mudança teria introduzido — o botão "Expulsar" estava hardcoded pra sempre chamar a API de GrupoTarefa; agora branca por `ConversaGrupoTipo` (Projeto → `GrupoProjetoAPI.expulsarMembro`). Também simplifiquei `chat/page.tsx#podeApagar` pra usar `MinhasPermissoes.PodeExcluirMensagens` (já calculado no backend) em vez de rederivar por papel hardcoded — isso também corrige o mesmo bug de fundo pra Tarefa (delegação de `PodeExcluirMensagens` não era refletida nessa função antes).
