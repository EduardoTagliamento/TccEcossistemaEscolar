...
## 🐛 Incidente separado (2026-08-12, mesmo dia): `gerarGUID()` gerando formato errado — corrigido

Não é sobre CPF vs. GUID — é sobre o **formato** do GUID em si, achado ao investigar "erro ao criar escola: EscolaGUID deve ter 36 caracteres".

**Causa raiz:** `backend/utils/helpers/guid.helper.ts` foi criado no mesmo commit desta migração (`65db764`, 2026-08-10) já como um gerador de **12 caracteres base64url**, achando que substituía um `uuidv4()` de 36 caracteres em TODO o sistema. Só que `usuario.UsuarioGUID` é deliberadamente `CHAR(12)` no banco — decisão real de `2026-08-10-usuario-guid-pk.ts` (LGPD: ID de usuário mais curto) — enquanto as outras ~44 tabelas com GUID (`escola`, `tarefaacademica`, `matricula`, etc.) são `CHAR(36)` de verdade. Um helper só, um formato só, pra dois requisitos incompatíveis.

**Efeito:** qualquer `INSERT` de entidade NÃO-usuario (escola, tarefa, etc.) começou a falhar na validação do próprio setter (`.length !== 36`) — sem gravar nada errado no banco, só quebrado. Confirmado via consulta direta em produção (agente MySQL): nenhuma linha malformada em nenhuma das ~170 colunas `CHAR(36)`/`VARCHAR(36)` do schema.

**Fix:** dois geradores, cada um pro seu domínio —
- `gerarGUID()` — voltou a ser UUID v4 padrão (`crypto.randomUUID()`, 36 chars) — usado por tudo, exceto `usuario`.
- `gerarGUIDUsuario()` (novo) — 12 chars base64url, só pra `usuario.UsuarioGUID`. 4 call sites que criavam usuário novo corrigidos: `usuario.service.ts` (x2), `escolaxusuarioxfuncao.service.ts`, `professor.service.ts`. Migration `2026-08-10-usuario-guid-pk.ts` (já executada em produção) também ajustada pra manter consistência se algum dia for re-rodada.
- `usuario.model.ts` ganhou validação de 12 caracteres no setter de `UsuarioGUID` (não tinha validação de tamanho nenhuma antes — só não quebrou por sorte, já que o `usuario` foi a única entidade sem esse guard).
- **Achado em cascata**: os schemas Zod de `grupoprojeto.schema.ts`/`grupotarefa.schema.ts` (mexidos hoje mais cedo, na correção dos bugs de adicionar/expulsar/transferir membro) validavam `UsuarioGUID`/`membroGUID`/`NovoLiderGUID` com o regex de UUID v4 (36 hex+hífen) — errado, esses campos guardam um `UsuarioGUID` de verdade (12 chars). Corrigido pra um `guidUsuario()` local em cada arquivo, mesmo padrão já usado (corretamente) em `escolaxusuarioxfuncao.schema.ts`.

**Verificado:** os outros 22 lugares que guardam `UsuarioGUID` como FK (via agente MySQL: `anexo`, `anotacao`, `matricula`, `notificacao`, `pendencia`, etc. — todos `CHAR(12)` no banco também) — os 14 setters de entidade que têm campo `UsuarioGUID` já eram todos "soltos" (sem checagem de tamanho), então nenhum deles quebra com o formato certo. `npx tsc --noEmit -p .`: 0 erros.

**Lição:** um helper de geração de ID compartilhado por múltiplos domínios com requisitos de formato genuinamente diferentes não deveria ser uma função só — nomear a variante explicitamente (`gerarGUIDUsuario` vs. `gerarGUID`) deixa o formato certo óbvio no call site, em vez de depender de todo mundo saber "ah, mas pra usuario é diferente".
...
## 🐛 Correção pós-conclusão (2026-08-12, mesmo dia) — bugs reais que a auditoria anterior classificou errado como "confirmados corretos"

A seção "Auditoria sistemática" (mais abaixo) tinha marcado `grupoprojeto.api.ts`, `grupotarefa.api.ts`, `types/projeto.ts` e `types/grupotarefa.ts` como **"confirmados corretos, sem alteração"** — essa conclusão estava **errada**. A auditoria anterior conferia só se o campo fazia sentido como CPF-por-design (tabela não migrada, ou "convidar por CPF" resolvido internamente); não conferia se o **formato validado no schema Zod batia com o que o service realmente consumia**. Uma checagem mais funda (ler `backend/schemas/*.schema.ts` junto com o service, não só o service) achou 4 endpoints genuinamente quebrados em produção:

**`grupoprojeto` — 3 endpoints quebrados (100% do tempo, desde que o schema de `usuarioxgrupoprojeto` migrou pra GUID):**
- `POST /:grupoGUID/membros` (adicionar membro) — schema validava `UsuarioCPF` como 11 dígitos, controller repassava esse valor direto pra `GrupoProjetoService.adicionarMembro(grupoGUID, membroGUID, ...)`, que faz `usuarioElegivel`/`usuarioPertenceAoGrupo`/`contarParticipacoesNoProjeto` — todas comparações/lookups por GUID real. Um CPF de 11 dígitos nunca bate com um GUID de 36 caracteres → sempre falhava.
- `DELETE /:grupoGUID/membros/:cpf` (expulsar membro) — mesmo padrão: rota validava `:cpf` como CPF, service (`expulsarMembro`) trata como `membroGUID`.
- `PATCH /:grupoGUID/transferir-lider` — mesmo padrão com `NovoLiderCPF`/`novoLiderGUID`. Ao contrário de `convitegrupoprojeto.enviarConvite` (que genuinamente resolve CPF→GUID via `findByCPF` antes de qualquer lookup — esse é CPF-por-design de verdade), `grupoprojeto.transferirLideranca` nunca fazia essa resolução.

**`grupotarefa` — 1 endpoint quebrado, 1 simplificado:**
- `DELETE /:grupoGUID/membros/:cpf` (expulsar membro) — mesmo bug exato do `grupoprojeto`: `:cpf` validado como CPF, `GrupoTarefaService.expulsarMembro` trata como GUID (`isMembroNaoLider(membroGUID, ...)`, `membroGUID === liderGUID`).
- `PATCH /:grupoGUID/transferir-lider` — este **não estava quebrado** (resolvia CPF→GUID de verdade via `findByCPF`, comentário explícito "UX de transferir por CPF"), mas o único consumidor real (`TransferirLiderancaModal.tsx`, radio-button escolhendo de uma lista de membros já carregada) só tinha `UsuarioGUID` disponível por membro — nunca CPF. Ou seja, a resolução CPF→GUID no backend era uma volta desnecessária: o cliente já tinha o GUID em mãos e tinha que fingir não ter. Simplificado pra aceitar `NovoLiderGUID` direto, mesmo padrão do `grupoprojeto` já corrigido.

**Correção (backend + frontend, `npx tsc --noEmit -p .` 0 erros nos dois depois):**
- `backend/schemas/grupoprojeto.schema.ts` — `GrupoAndMembroParamsSchema.cpf`/`AdicionarMembroBodySchema.UsuarioCPF`/`TransferirLiderBodySchema.NovoLiderCPF` → `membroGUID`/`UsuarioGUID`/`NovoLiderGUID`, todos `guid()`. Helper `cpfCampo` removido (não sobrou nenhum uso legítimo no arquivo).
- `routes/grupoprojeto.routes.ts` + `backend/controllers/grupoprojeto.controller.ts` — `:cpf` → `:membroGUID`, destructuring dos 3 handlers atualizado.
- `backend/schemas/grupotarefa.schema.ts` — `GrupoAndMembroParamsSchema.cpf` → `membroGUID`; `TransferirLiderBodySchema.NovoLiderCPF` → `NovoLiderGUID` (guid, não mais cpfParam). `cpfParam`/`CPF_REGEX` removidos (sem uso restante no arquivo).
- `routes/grupotarefa.routes.ts` + `backend/controllers/grupotarefa.controller.ts` — mesmo padrão pros 2 handlers.
- `backend/services/grupotarefa.service.ts` — `transferirLideranca` não resolve mais CPF internamente, recebe `novoLiderGUID` direto (removida a chamada a `#usuarioDAO.findByCPF`).
- Frontend: `types/projeto.ts` (`Projeto.UsuarioCPFCriador`→`UsuarioGUIDCriador`, `MembroGrupoProjeto.UsuarioCPF`→`UsuarioGUID`, `GrupoProjeto.UsuarioCPFLider`→`UsuarioGUIDLider`, `ConviteGrupoProjeto.LiderCPF`→`LiderGUID`/`UsuarioCPFConvidado`→`UsuarioGUIDConvidado`), `types/grupotarefa.ts` (mesmo padrão + `TransferirLiderancaData.NovoCPFLider`→`NovoLiderGUID`), `lib/api/grupoprojeto.api.ts`, `lib/api/grupotarefa.api.ts`, `lib/grupoprojeto/useGrupoProjetoMutations.ts`, `lib/grupotarefa/useGrupoTarefaMutations.ts`, `components/TransferirLiderancaModal.tsx`, `components/SolicitarEntradaModal.tsx` (só `key` prop), `app/dashboard/[escolaGUID]/projetos/[projetoGUID]/page.tsx`, `.../projetos/[projetoGUID]/grupos/[grupoGUID]/page.tsx` (+ "Adicionar membro" passou a resolver CPF digitado → GUID via `buscarUsuarioPorCPF` antes de chamar a mutation, em vez de mandar CPF cru — o service não resolve mais), `.../tarefas/[tarefaGUID]/page.tsx`.
- Achados menores da mesma varredura, sem bug funcional (só tipo mentindo sobre o shape, sem consumidor lendo o campo errado): `frontend/lib/api/horarioturma.api.ts` (`HorarioTurma`/`BancoItem.UsuarioCPF`→`UsuarioGUID`), `frontend/lib/api/categoriaconteudo.api.ts` (`CategoriaConteudo.UsuarioCPF`→`UsuarioGUID`), `frontend/lib/api/materiasmodulo.api.ts` (`MateriaCustomizacao.UsuarioCPF`→`UsuarioGUID`).

**Lição:** "auditoria confirma que o tipo está certo" não é o mesmo que "auditoria confirma que schema de validação e service concordam sobre o formato do dado". Uma auditoria futura desse tipo precisa ler os 3 arquivos juntos (schema Zod + controller + service), não só o service isoladamente — o `tsc` nunca vai pegar isso porque tanto CPF quanto GUID são `string`.
...
>
> ⚠️ **Correção (2026-08-12, mesmo dia):** os itens riscados acima estavam **errados** — essa auditoria só olhou o service, não o schema Zod da rota. Ver seção "🐛 Correção pós-conclusão" no topo do arquivo pros bugs reais achados e corrigidos (`grupoprojeto`: adicionar/expulsar/transferir membro todos quebrados; `grupotarefa`: expulsar membro quebrado, transferir liderança desnecessariamente indireto).
